...
def render_result(bases, bases_feat, loc_gt, loc_est, scale=30, border=1):
    ...
    cv2.circle(
        canvas,
        tuple(np.round(loc_gt).astype(int)),
        int(scale * 0.1),
        [0, 0, 255],
        -1,
    )  # 0.1 meter circle
    cv2.circle(
        canvas, tuple(np.round(loc_gt).astype(int)), int(scale * 0.5), [0, 0, 255]
    )  # 0.5 meter circle
    cv2.circle(
        canvas, tuple(np.round(loc_gt).astype(int)), int(scale), [0, 0, 255]
    )  # 1 meter circle
    cv2.drawMarker(
        canvas,
        tuple(np.round(loc_est).astype(int)),
        [255, 255, 255],
        cv2.MARKER_CROSS,
        int(scale * 0.2),
        1,
...
def match_images(
    ...
    for i in range(n_images):
        ...
        if mode in ["refine", "match"]:
            ...
            rot_est = (rot_samples[rot_est.cpu()] / 180 * np.pi).to(scores.device)
