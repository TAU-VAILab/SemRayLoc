...
parser.add_argument("--gpu", type=str, default="1", help="Device ID of the GPU to use.")
parser.add_argument(
...
from eval_utils import *  # This imports functions like sample_floorplan, match_images, render_result, etc.

def mkdir_if_not_exist(path):
    if not os.path.isdir(path):
        os.makedirs(path)

# Modified plotting function to generate a heatmap overlay with ground truth and estimated pose arrows.
...
if __name__ == "__main__":
    with open(os.path.join(opt.log_dir, "cfg.json"), "r") as f:
        cfg = json.load(f)
    # Overwrite some cfg if needed
    if opt.fov is not None:
        cfg["fov"] = opt.fov
    assert not (isinstance(cfg["fov"], list) or isinstance(cfg["fov"], tuple)), \
    ...
    if opt.save:
        ...
        # Create folder for heatmap visualizations.
        mkdir_if_not_exist(os.path.join(save_dir, "heatmaps"))
    ...
    for data in dataloader:
        ...
        for k in data.keys():
            if torch.is_tensor(data[k]) and not data[k].is_cuda:
                data[k] = data[k].cuda()
        if cfg.get("disable_semantics", False):
            data["bases_feat"][..., -2:] = 0
        ...
        print(
            "{idx}, {sampling_fps:.0f} sampling_fps, {sampling_time:.2f} sampling_time, "
            "{matching_fps:.2f} matching_fps, {matching_time:.2f} matching_time, "
            "{median_terr:.4f} median_terr, {median_rerr:.4f} median_rerr".format(**fmt)
        )

        for i in range(match_ret["n_images"]):
            # if (idx, i) not in [(0, 3), (1, 0), (5, 3), (0, 3), (9, 11)]:
            #     continue
            score_map = match_ret["score_maps"][i]
            rot_map = match_ret["rot_maps"][i]
            loc_gt = match_ret["loc_gts"][i]
            loc_est = match_ret["loc_ests"][i]
            # Retrieve the rotations as well.
            rot_gt = match_ret["rot_gts"][i]
            rot_est = match_ret["rot_ests"][i]
            ...
            if opt.save:
                if i == 0:
                    ...
                    with open(
                        ...
                        merged_raw["idx"] = idx
                        merged_raw["samples_feat"] = None  # too large to store, render in runtime
                ...
                cv2.imwrite(
                    os.path.join(save_dir, "score_maps", f"score_map_{idx:04d}_{i:03d}.png"),
                    score_map_viz,
                )
                cv2.imwrite(
                    os.path.join(save_dir, "rot_maps", f"rot_map_{idx:04d}_{i:03d}.png"),
                    rot_map_viz,
                )
                cv2.imwrite(
                ...
                cv2.imwrite(
                    os.path.join(save_dir, "query_images", f"query_image_{idx:04d}_{i:03d}.png"),
                    query_image_viz,
                )
                # --- NEW: Save an additional heatmap image. ---
                # Combine location and rotation so that each pose is a 3-element vector.
                gt_pose = np.concatenate([loc_gt, rot_gt], axis=0)   # [x, y, theta]
                pred_pose = np.concatenate([loc_est, rot_est], axis=0)  # [x, y, theta]
                heatmap_output_path = os.path.join(save_dir, "heatmaps", f"heatmap_{idx:04d}_{i:03d}.png")
                # Use the raw score_map as the probability volume.
                # plot_single_map(score_map, gt_pose, pred_pose, opt.sample_grid, heatmap_output_path, device='cpu')
            else:
