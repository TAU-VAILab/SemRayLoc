...
class S3dDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Use a try/except block to catch missing files or other errors.
        try:
            ...
            bases_feat = np.concatenate(
                [
                    (bases - bases.mean(axis=0, keepdims=True)) / 5.0,
                    np.zeros_like(bases[:, 0:1]),
                    bases_normal,
                    bases_door_mask.reshape(-1, 1),
            ...
            for pano_node in pano_node_list:
                ...
                if not os.path.exists(cam_xyz_path):
                    print(f"Missing file: {cam_xyz_path}")
                    raise FileNotFoundError(f"Missing camera_xyz.txt at {cam_xyz_path}")

                pano_loc = (
                    np.loadtxt(cam_xyz_path)[:2] / 1000.0
                )
                pano_loc = rot_verts(pano_loc, global_rot)
                pano_rot = global_rot
                render_type = "full" if self.is_training else self.s3d_eval_furnishing
                ...
                if pano_image is None:
                    print(f"Could not load image: {pano_image_path}")
                    if self.return_all_panos:
                        continue
                    else:
                        raise FileNotFoundError(f"Missing image: {pano_image_path}")
                pano_image = cv2.flip(pano_image, 1)

                rnd_rot = (np.random.rand() if self.is_training else 0) * 360
