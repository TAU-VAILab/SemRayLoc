...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="Train_models/configurations/full/weight_prediction_net_config.yaml",
    ...
    model_log_dir = os.path.join(config.ckpt_path, f'weight_predictor_net_epochs-{str(config.epochs)}_net_size-{config.NET_SIZE}_acc_only-{config.acc_only}-loss_type-{config.loss_type}-v4')
    ...
    experiment_name = f"weight_predictor_net_{num_trainable_params_million:.3f}M_dataset_{config.DATA_SET_SIZE}_batch_size-{config.batch_size}_acc_only-{config.acc_only}_net_size-{config.NET_SIZE}-loss_type-{config.loss_type}-v4"
    ...
    trainer.save_checkpoint(os.path.join(model_log_dir, "final_weight_predictor_model_checkpoint.ckpt"))
