...
def measure_similarity(
    ...
    """
    # Adjust candidate semantics (as in your original code)
    # cand_semantics_copy = cand_semantics.copy()
    # cand_semantics_copy[cand_semantics_copy == 2] = -1
    # cand_semantics_copy[cand_semantics_copy == 1] = 2
    # cand_semantics_copy[cand_semantics_copy == -1] = 1
...
def evaluate_room_aware_with_refine(config):
    ...
    if not config.use_ground_truth_depth or config.prediction_type in ["depth", "combined"]:
        depth_net = depth_net_pl_adaptive.load_from_checkpoint(
        ...
        for data_idx in tqdm.tqdm(range(len(test_set)), desc="Samples", leave=False):
            ...
            if config.use_room_aware:
                ...
                if max_prob.item() > config.room_selection_threshold:
                    id_to_room_type = {v: k for k, v in zind_room_type_to_id.items()} # For zind
                    predicted_room = id_to_room_type[room_idx.item()]
                    # retrieve polygons
                    room_polygons = data["room_polygons"].get(predicted_room, [])
                
                # Using room polygons for room-aware version
            ...
            #     # if predicted_room != 'None' and predicted_room != 'bedroom' and predicted_room != 'bathroom':
            #     # plot_prob_dist(prob_dist_pred, save_path=results_type_dir, file_name=f"{str(scene)}_{idx_within_scene}-with_refine" , pose_pred=[refine_x,refine_y,refine_o], ref_pose_map=ref_pose_map)
            #     # print(f"{idx_within_scene}-{str(scene)}_with_refine acc: {refine_trans} oren: {refine_rot}")
