...
from utils.localization_utils import (
    ...
    get_ray_from_semantics_soft,  
    localize_soft_semantics,
    localize,
    ...
    for depth_weight, semantic_weight in tqdm.tqdm(config.weight_combinations, desc="Weight combinations"):
        ...
        for data_idx in tqdm.tqdm(range(len(test_set)), desc="Samples", leave=False):
            ...
            if not config.use_ground_truth_semantic:
                ...
                pred_sem_rays_prob = get_ray_from_semantics_soft(
                    prob_squeezed,
                    angle_between_rays=80/40,
                    desired_ray_count=9,
                    window_size=1
                )
                ...
                pred_rays_semantic = get_ray_from_semantics_soft(sampled_semantic_indices_np)
            ...
            if not config.use_saved_prob_vol:
                ...
                prob_vol_pred_semantic, _, _, _ = localize_soft_semantics(
                    torch.tensor(semantics[scene]["desdf"]),
                    torch.tensor(pred_sem_rays_prob, device="cpu"),
                    return_np=False,
                    localize_type="sem",
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="evaluation/configuration/S3D/config_eval_soft_semantic_exp.yaml",
