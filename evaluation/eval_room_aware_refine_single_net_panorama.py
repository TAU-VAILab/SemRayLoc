...
from data_utils.data_utils_for_laser_train_s3d import GridSeqDataset
        ...
        for ang in ray_angles:
            ...
            if cache_key in candidate_cache:
                ...
                depth_val_m, prediction_class, _, _ = ray_cast(
                    semantic_map, candidate_pos_pixels, ang, dist_max=15 * 100, min_dist=5, cast_type=2
                )
...
def evaluate_room_aware_with_refine(config):
    ...
    desdfs, semantics, maps, gt_poses, valid_scene_names, walls = load_scene_data(
        test_set, config.dataset_dir, config.desdf_path, use_walls= False
    )
        ...
        for data_idx in tqdm.tqdm(range(len(test_set)), desc="Samples", leave=False):
            ...
            if config.use_room_aware:
                ...
                final_prob_vol, prob_dist_pred, orientation_map, pose_pred = finalize_localization(
                    ...
                    combined_prob_vol, [], room_polygons
                )
...
def main():
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="evaluation/configuration/S3D/config_eval_panorama.yaml",
    ...
    config_file_path = os.path.join(config.results_dir, "config.yaml")
