    ...
    for data_idx in tqdm.tqdm(range(len(test_set))):
        ...
        for depth_weight, semantic_weight in weight_combinations:
            ...
            if config.use_room_aware:
                room_logits, room_probs = room_type_model(torch.tensor(data["ref_img"], device=device).unsqueeze(0))
                class_probs = torch.softmax(room_logits, dim=1)  # shape: [1, 16]
                max_prob, room_idx = torch.max(class_probs, dim=1)
                
                # Only use room polygons if the max probability is above the configured threshold
                if max_prob.item() > config.room_selection_threshold:
                    # Create inverse mapping from room id to room type string
                    id_to_room_type = {v: k for k, v in room_type_to_id.items()}
                    predicted_room = id_to_room_type[room_idx.item()]
                    # Retrieve the polygons corresponding to the predicted room type
                    room_polygons = data["room_polygons"].get(predicted_room, [])
                else:
                    room_polygons = []  # or set to None if that is preferred

                _, prob_dist_pred, _, pose_pred = finalize_localization(combined_prob_vol_pred, data["room_polygons"], room_polygons, )                
            else:                
                _, prob_dist_pred, _, pose_pred = finalize_localization(combined_prob_vol_pred)
            ...
            plot_prob_dist(prob_dist_pred, save_path=results_type_dir, file_name=f"{str(scene)}-{data_idx}" , pose_pred=pose_pred, ref_pose_map=ref_pose_map)
...
def evaluate_observation(prediction_type, config, device):
    ...
    evaluate_combined_model(
        ...
        valid_scene_names,
        config=config,  
        room_type_model=room_type_model,
    )
