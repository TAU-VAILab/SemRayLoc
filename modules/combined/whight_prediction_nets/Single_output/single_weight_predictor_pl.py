...
from modules.combined.whight_prediction_nets.Single_output.single_weight_predictor_net import SingleWeightPredictorNet
import torch
...
class SingleWeightPredictorPL(LightningModule):
    def __init__(self, config, lr=1e-4, log_dir='logs', net_size='small', acc_only=True):
        super().__init__()
        self.save_hyperparameters()
        self.model = SingleWeightPredictorNet(net_size=net_size)
        self.lr = lr
    ...
    def training_step(self, batch, batch_idx):
        ...
        weights = self(depth_map, semantic_map)  # Outputs are weights
        w_depth = weights[:, 0]
        w_semantic = weights[:, 1]
        w_depth = w_depth.view(-1, 1, 1)  # Shape: [B, 1, 1]
        w_semantic = w_semantic.view(-1, 1, 1)  # Shape: [B, 1, 1]
