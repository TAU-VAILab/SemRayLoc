...
# Full Weight Predictor Network
class FullWeightPredictorNet(nn.Module):
    def __init__(self, net_size='medium'):
        super(FullWeightPredictorNet, self).__init__()
        ...
        if net_size == 'small':
            ...
            self.output_layer = nn.Conv3d(16, 2, kernel_size=1)
            ...
            self.conv_layers_before_att = nn.Sequential(
                nn.Conv3d(in_channels=2, out_channels=16, kernel_size=3, padding=1),
                nn.ReLU(inplace=True),
                nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2)),  # Downsample H and W by 2
                nn.Conv3d(16, 32, kernel_size=3, padding=1),
                nn.ReLU(inplace=True),
                nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2)),  # Downsample H and W by another 2
            ...
            # Final output layer
            self.output_layer = nn.Conv3d(32, 2, kernel_size=1)
        ...
        for module in self.modules():
            if isinstance(module, nn.Conv3d):
                if module is self.output_layer:
                    # Initialize output layer weights and biases to zero
                    nn.init.constant_(module.weight, 0)
                    if module.bias is not None:
                        nn.init.constant_(module.bias, 0)
                else:
                    # Use Kaiming (He) initialization for other layers
                    nn.init.kaiming_normal_(module.weight, nonlinearity='relu')
                    if module.bias is not None:
                        nn.init.constant_(module.bias, 0)
            elif isinstance(module, SelfAttention3D):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 4, 2, 3)  # x is [B, C, O, H, W]

        if self.net_size == 'medium' or self.net_size == 'large':
        ...
        # Output layer
        weight_map = self.output_layer(x)  # [B, 2, O, H, W]

        # Permute back to [B, 2, H, W, O]
        weight_map = weight_map.permute(0, 1, 3, 4, 2)  # [B, 2, H, W, O]

        # Apply softmax over the 'type' dimension (depth and semantic)
        weight_map = F.softmax(weight_map, dim=1)  # [B, 2, H, W, O]

        # Split the weight maps
        weight_map1 = weight_map[:, 0, :, :, :]  # [B, H, W, O]
        weight_map2 = weight_map[:, 1, :, :, :]  # [B, H, W, O]
