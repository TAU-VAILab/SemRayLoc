    ...
    def forward(self, x, emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                x = layer(x)
            #print(f"[TimestepEmbedSequential] Layer {layer.__class__.__name__}, output shape: {x.shape}")
        return x
...
class ResBlock(TimestepBlock):
    def __init__(self, channels, emb_channels, dropout, out_channels=None, 
        ...
        if out_channels == channels:
            ...
            #print(f"[ResBlock] skip_connection is Identity")
        elif use_conv:
            self.skip_connection = conv_nd(dims, channels, out_channels or channels, 3, padding=1)
            #print(f"[ResBlock] skip_connection is conv with kernel_size 3")
        else:
            self.skip_connection = conv_nd(dims, channels, out_channels or channels, 1)
            #print(f"[ResBlock] skip_connection is conv with kernel_size 1")
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            #print(f"[ResBlock] h after in_layers[:-1]: {h.shape}")
            h = self.h_upd(h)
            #print(f"[ResBlock] h after h_upd: {h.shape}")
            x = self.x_upd(x)
            #print(f"[ResBlock] x after x_upd: {x.shape}")
            h = self.in_layers[-1](h)
            #print(f"[ResBlock] h after in_layers[-1]: {h.shape}")
        else:
            h = self.in_layers(x)
            #print(f"[ResBlock] h after in_layers: {h.shape}")
        emb_out = self.emb_layers(emb).type(h.dtype)
        #print(f"[ResBlock] emb_out after emb_layers: {emb_out.shape}")
        ...
        if self.use_scale_shift_norm:
            ...
            #print(f"[ResBlock] h after scale_shift_norm: {h.shape}")
        else:
            h = h + emb_out
            #print(f"[ResBlock] h after adding emb_out: {h.shape}")
            h = self.out_layers(h)
            #print(f"[ResBlock] h after out_layers: {h.shape}")
        out = self.skip_connection(x) + h
        #print(f"[ResBlock] skip_connection(x): {self.skip_connection(x).shape}, out: {out.shape}")
        return out
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        x = x.reshape(b, c, -1)
        x = self.norm(x)
        #print(f"[AttentionBlock] norm output shape: {x.shape}")
        qkv = self.qkv(x)
        #print(f"[AttentionBlock] qkv shape: {qkv.shape}")
        h = self.attention(qkv)
        #print(f"[AttentionBlock] attention output shape: {h.shape}")
        h = self.proj_out(h)
        #print(f"[AttentionBlock] proj_out output shape: {h.shape}")
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        #print(f"[QKVAttention] q shape: {q.shape}, k shape: {k.shape}, v shape: {v.shape}")
        ...
        #print(f"[QKVAttention] weight shape: {weight.shape}")
        a = th.einsum("bts,bcs->bct", weight, v.view(bs * self.n_heads, ch, length))
        out = a.view(bs, -1, length)
        #print(f"[QKVAttention] output shape: {out.shape}")
        return out

class UNetModel_acc_only(nn.Module):
    """
    Adjusted UNet model for the acc_only case, handling 2D data.
    """
    def __init__(
        self,
        in_channels,
                ...
                if ds in attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ...
                            dims=dims,  # Overall model dimension
                        )
                    )
        ...
        # Modify self.out to include the normalization
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(dims, input_ch, self.out_channels, 3, padding=1)),
            # NormalizeOutput(),  # Add the custom normalization module
        )
    ...
    def forward(self, x, timesteps, cond_input):
        ...
        :param x: an [N x 1 x H x W] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param cond_input: an [N x 2 x H x W] Tensor of conditional inputs.
        :return: an [N x 1 x H x W] Tensor of outputs.
        """
        # Concatenate conditions to the input
        x = th.cat([x, cond_input], dim=1)  # Now x has shape [N, 1 + 2, H, W]
        #print(f"[UNetModel] Input x shape after concatenation: {x.shape}")
        timesteps = timesteps.to(x.device)
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        #print(f"[UNetModel] Timestep embedding shape: {emb.shape}")

        hs = []
        h = x.type(self.dtype)
        for idx, module in enumerate(self.input_blocks):
            h = module(h, emb)
            #print(f"[UNetModel] After input block {idx}, h shape: {h.shape}")
            hs.append(h)
        h = self.middle_block(h, emb)
        #print(f"[UNetModel] After middle block, h shape: {h.shape}")
        for idx, module in enumerate(self.output_blocks):
            h_skip = hs.pop()
            #print(f"[UNetModel] Output block {idx}, h shape: {h.shape}, h_skip shape: {h_skip.shape}")
            ...
            if h.shape[2:] != h_skip.shape[2:]:
                ...
                #print(f"[UNetModel] After interpolation, h shape: {h.shape}")
            h = th.cat([h, h_skip], dim=1)
            #print(f"[UNetModel] After concatenation with skip connection, h shape: {h.shape}")
            h = module(h, emb)
            #print(f"[UNetModel] After output block {idx}, h shape: {h.shape}")
        h = h.type(x.dtype)
        out = self.out(h)
        #print(f"[UNetModel] Output of UNetModel, out shape: {out.shape}")
