...
def copy_and_rename_images(scene_id, base_path, output_base_path):
    ...
    for traj in sorted(os.listdir(scene_base_path)):
        ...
        panorama_dir = os.path.join(traj_dir, 'panorama', 'full')
        if not os.path.isdir(panorama_dir):
            continue

        for file in sorted(os.listdir(panorama_dir)):
            if file.endswith('.png') and 'rgb_rawlight' in file:
                source_file = os.path.join(panorama_dir, file)
                dest_file = os.path.join(rgb_output_path, f"{image_counter}.png")
                shutil.copyfile(source_file, dest_file)
                image_counter += 1


def create_raycast_file(camera_positions, img_walls, img_semantic,
    ...
    for i, camera_info in enumerate(camera_positions):
        ...
        for j, ang in enumerate(angs):
            # Ray 1: walls
            dist, _, hit_coords_walls, _ = ray_cast(
                img_walls, np.array([camera_x_walls, camera_y_walls]), ang
            )
            # Ray 2: semantic
            _, prediction_class, hit_coords_semantic, normal = ray_cast(
                img_semantic, np.array([camera_x_semantic, camera_y_semantic]), ang
            )
...
def create_pose_file(camera_positions, output_path):
    """
    """
    poses_file = os.path.join(output_path, 'poses.txt')

    with open(poses_file, 'w') as pf:
        for i, camera_info in enumerate(camera_positions):
            yaw_rad = 0
            camera_x_semantic = camera_info['vx_semantic']
            camera_y_semantic = camera_info['vy_semantic']
            pf.write(f"{camera_x_semantic} "
                     f"{camera_y_semantic} "
                     f"{yaw_rad}\n")
...
def process_scene(scene_id, base_path, output_base_path,
    ...
    # print("Semantic bbox:", semantic_bbox)
    camera_positions = []
    scene_base_path = os.path.join(base_path, f'scene_{scene_id.zfill(5)}', '2D_rendering')
    ...
    for traj in sorted(os.listdir(scene_base_path)):
        traj_dir = os.path.join(scene_base_path, traj)
        pano_dir = os.path.join(traj_dir, 'panorama')
        if not os.path.isdir(pano_dir):
            continue

        camera_pose_file = os.path.join(pano_dir, 'camera_xyz.txt')
        if not os.path.exists(camera_pose_file):
            continue

        room_id = os.path.basename(traj_dir)
        resolution_mm_per_pixel = resolution * 1000

        with open(camera_pose_file, 'r') as file:
            for line in file:
                try:
                    (vx_raw, vy_raw, vz) = map(float, line.strip().split())
    ...
    create_pose_file(camera_positions, os.path.join(output_base_path, f'scene_{int_scene_id}'))
