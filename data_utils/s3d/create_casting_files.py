...
def create_raycast_file(camera_positions, img_semantic,
    ...
    for i, camera_info in enumerate(camera_positions):
        ...
        for j, ang in enumerate(angs):
            dist, pred_class, hit_coord = ray_cast(img_semantic, np.array([camera_x_semantic, camera_y_semantic]),
...
def create_roomtype_data(camera_positions, room_type_polygons,  # dictionary
    ...
    # 1) Define a small priority map for the order of polygons (i.e the libing room wont be over the bedroom)
    priority_map = {
        "bedroom": 9,
        "living room": 10
    }
...
def process_scene(scene_id, base_path, output_base_path,
    ...
    if not os.path.exists(floorplan_semantic_path):
