...
def plot_camera_positions_and_rays(img, hits, semantic, ref_pose, output_path,image,file_name):
    ...
    fig1, ax1 = plt.subplots(figsize=(img_width / 100, img_height / 100), dpi=100)
    ax1.imshow(img)
    x,y = ref_pose[:2]
...
class GridSeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f"ref_pose[2]:{ref_pose[2]}")
        # print(f"pano_rot:{pano_rot}")
        depth_ref =[]
        semantic_ref = []
        hit_coords = []
        ...
        plot_camera_positions_and_rays(img_semantic,hit_coords,semantic_ref,np.array([x*100, y*100]),"/home/yuvalg/projects/Semantic_Floor_plan_localization/temp", query_image,file_name=f"{scene_name}_{idx_within_scene}_yaw-{yaw}")        
        ...
        if is_zind:
            room_label_id = zind_room_type_to_id.get(room_label_str, room_type_to_id["undefined"])
        else:
