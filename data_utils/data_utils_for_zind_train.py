...
def eval_valid_floors(dataset_dir, set):
    valid_floors = []
    for idx in set:
        instance_path = os.path.join(dataset_dir, f"{idx:04d}")
...
class ZindDataset(Dataset):
    def __init__(
        ...
        with open("/datadrive2/CRM.AI.Research/TeamFolders/Email/repo_yuval/FloorPlan/Semantic_Floor_plan_localization/laser/dataset/zind_partition.json", "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        instance_path = os.path.join(self.dataset_dir, f"{zind_idx:04d}")
        json_path = os.path.join(instance_path, "zind_data.json")
        with open(json_path, "r") as f:
            data = json.load(f)
        ...
        # FOR our pipline 
        rot_rad = gt_rots[0][0]
        scene_dir = os.path.join(self.conf.fp_path, f"scene_{zind_idx:01d}_{floor_name}")
        ...
        angs = center_angs + rot_rad
        angs = (np.radians(90) - (angs)[::-1]) % (2 * np.pi)
        
        depth_ref =[]
        semantic_ref = []
        for _, ang in enumerate(angs):
            dist, pred_class, _, _ = ray_cast(img_semantic, np.array([x*100, y*100]),
                                                ang, dist_max=15*100, min_dist=80,  cast_type = 2)
