...
for scene in tqdm.tqdm(scenes):
    ...
    low_res_img_dir = os.path.join(data_dir, scene, "rgb_low_res")
    
    if not os.path.exists(img_dir):
        continue  # skip if the 'rgb' directory does not exist
    if not os.path.exists(low_res_img_dir):
    ...
    for img in sorted(os.listdir(img_dir)):
        ...
        im_resized = cv2.resize(im, (228, 128))
        print(f"found image to resize: scene:{scene} number: {n_img}")
        # Save the resized image as a .png file
        cv2.imwrite(os.path.join(low_res_img_dir, f"{n_img}.png"), im_resized)
